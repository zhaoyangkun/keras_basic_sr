...
def learned_perceptual_metric_model(image_size, vgg_model_ckpt_fn, lin_model_ckpt_fn):
    ...
    lin_out = [
        Lambda(lambda x: tf.reduce_mean(x, axis=[2, 3], keepdims=False))(t)
        for t in lin_out
    ]
...
def calculate_lpips(image_size, batch_ref, batch_inp):
    """
    计算 lpips 指标
    """
    model_dir = "./model"
    ...
    if len(batch_ref.shape) == 3:
        batch_ref = tf.expand_dims(batch_ref, 0)
    if len(batch_inp.shape) == 3:
        batch_inp = tf.expand_dims(batch_inp, 0)
